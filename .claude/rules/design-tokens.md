...
| Nordic Sun Light | see TODO | `--color-sun-light` | Hover state on white backgrounds |
| Nordic Fjord | `#00A7B5` | `--color-fjord` | Non-interactive weight: icons, key figures, badges |

> TODO(decision needed): `04 §1.2` defines Nordic Sun Light as `#FFEA8F`. `04 §1.6.1`,
> `§4.1.1` and `§4.2` all say `#FFD100`. These are different colours. Pick one.

> TODO(decision needed): `04 §1.2` lists the variable as `--color-sun-ligh` (truncated).
> Written here as `--color-sun-light`. Confirm.
...
`#B0CCD9` `--color-data-3` · `#D9B0B0` `--color-data-4` Nordic Berry ·
`#C4B8D9` `--color-data-5` Nordic Heather

> TODO(decision needed): `04 §1.5` names `--color-data-3` "Nordic Fjord", colliding with
> the accent colour of the same name at `#00A7B5`. One of the two needs renaming.

### Applying colour

- Primary CTA background is Nordic Sun. Never another colour.
- Accent colours are for interactive elements. Never decoration.
...
> TODO(decision needed): `04 §1.6.1` says *"Reserve accent colours for interactive
> elements — not decoration"*, while `04 §6.3` assigns Nordic Fjord `#00A7B5` — an
> accent colour — to *"decorative/informational use"* on icons. The two cannot both
> hold. Decide whether Nordic Fjord is exempt from the accent rule, or whether
> informational icons use a neutral instead.
